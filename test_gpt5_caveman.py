...
def run(prompt):
    ...
    if r.get("status") == "success":
        usage = r.get("usage", {})
        return r.get("generated_text", ""), elapsed, r.get("cost", 0), usage.get("prompt_tokens", 0), usage.get("completion_tokens", 0)
    return "ERROR", elapsed, 0, 0, 0

print("Model: GPT-5 + Caveman Mode\n" + "=" * 50)
