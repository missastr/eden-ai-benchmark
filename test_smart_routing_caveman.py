...
def run(prompt):
    ...
    return content, elapsed, cost, in_tok, out_tok, model_used

print("Model: Smart Routing (10 models) + Caveman Mode\n" + "=" * 50)

total_cost = total_time = 0
for task in TASKS:
    content, elapsed, cost, in_tok, out_tok, model_used = run(task["prompt"])
    total_cost += cost
    total_time += elapsed
    print(f"\n[{task['type']}]")
